...
int WavInFile::deinterleaveRead(short *buffer, int maxElems) {
	short *inBuffer = (short*)malloc(maxElems*sizeof(short));
	int numElems = read(inBuffer, maxElems);
	int hMaxElems = maxElems*.5;
	for (int i=0; i<numElems; i+=2) {
		buffer[(int)(i*.5)] = inBuffer[i];
		buffer[(int)(i*.5)+hMaxElems] = inBuffer[i+1];
	}
	free(inBuffer);
	return numElems;
}
